...
### Load data:
overall_bcx <- vroom("overall_inc_bcx_wbc_rbc_platelet_harmonised_combined_sffdr.tsv.gz")
overall_clumped <- vroom("overall_inc_clumped.valid.snp", delim = "\t", col_names = FALSE)
...
pdf("overall_inc_ch_bcx_sffdr_significance_plot.pdf")
...
sffdr_results <- data.frame(SNP = rsid$rsid,
                            ...
                            fpi0 = fpi0)
head(sffdr_results)
write.table(sffdr_results, file = "overall_inc_ch_bcx_eur_sffdr_results.txt", row.names = FALSE, col.names = TRUE, quote = FALSE)