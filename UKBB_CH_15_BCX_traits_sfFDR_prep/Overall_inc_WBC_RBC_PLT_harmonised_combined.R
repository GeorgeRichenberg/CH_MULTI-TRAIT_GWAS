...
### Load data:
overall_wbc <- vroom("overall_inc_bcx_wbc_harmonised_combined.tsv.gz")
overall_rbc <- vroom("overall_inc_bcx_rbc_harmonised_combined.tsv.gz", col_select = -c(rsid, CHR, BP, ea_overall, oa_overall, eaf_overall, beta_overall, se_overall, pval_overall))
overall_platelet <- vroom("overall_inc_bcx_platelet_harmonised_combined.tsv.gz", col_select = -c(rsid, CHR, BP, ea_overall, oa_overall, eaf_overall, beta_overall, se_overall, pval_overall))
...
overall_bcx2 <- overall_bcx %>%
 ...
 ### Save output:
vroom_write(overall_bcx, file = "overall_inc_bcx_wbc_rbc_platelet_harmonised_combined.tsv.gz")
vroom_write(overall_bcx2, file = "overall_inc_bcx_wbc_rbc_platelet_harmonised_combined_sffdr.tsv.gz")
...
for (trait in traits) {
  ...
  output_file <- paste0(trait, "_overall_inc.sumstats.txt")
